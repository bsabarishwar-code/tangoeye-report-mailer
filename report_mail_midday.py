...
input_date     = yesterday.strftime("%d-%m-%Y")    # DD-MM-YYYY
input_date_iso = yesterday.strftime("%Y-%m-%d")    # YYYY-MM-DD

EXCLUDED_IDS = {"201", "425", "458", "468", "520"}

# 59 key clients — shown in the top table
KEY_CLIENT_NAMES = [
...
# Retry: check every hour, up to 6 PM IST (6 retries after the 12 PM trigger)
MAX_RETRIES = 6
...
print(f"Midday check for: {input_date}")
...
def send_token_alert():
    ...
    <p>The midday report for <b>{input_date}</b> was generated using <b>OpenSearch</b> as a fallback.</p>
    <p>To restore normal operation, update the token:</p>
    <ol>
      <li>Log in to the dashboard → open Chrome DevTools → Network tab</li>
      <li>Copy the <code>Authorization: Bearer &lt;token&gt;</code> value</li>
      <li>Open the GitHub repo → edit <code>report_mail_midday.py</code></li>
      <li>Replace the default value on line 8 with the new token</li>
      <li>Commit — the next run will use the new token</li>
    </ol>
    """
...
def fetch_from_dashboard():
    ...
    if go_colors_id:
        ...
        print("  Go Colors not found in client list.")
        return None, None

    # Send logs for sent clients
...
def fetch_from_opensearch():
    ...
    if not go_colors_sent:
        ...
        return None, None
    print("  Go Colors confirmed sent (OpenSearch) — proceeding.")
...
# ── RETRY LOOP: check every hour, up to 6 PM IST ─────────────────────────────
...
for attempt in range(MAX_RETRIES + 1):
    ...
    if rows is not None:
        print(f"  Go Colors confirmed — sending midday email.")
        break

    if attempt < MAX_RETRIES:
        next_check = now_ist + timedelta(hours=1)
        print(f"  Go Colors not sent. Next check at {next_check.strftime('%H:%M IST')} (sleeping 1 hour)...")
        time.sleep(3600)
    else:
        print("Go Colors not sent by 6:00 PM IST. No midday email sent today.")
        raise SystemExit(0)
...
print(f"  Total rows: {len(rows)}  Key: {len(key_rows)}  Other: {len(other_rows)}  Source: {data_source}")
...
# ── SEND ──────────────────────────────────────────────────────────────────────
send_brevo(f"Midday Report Check - {input_date}", html, RECEIVER_EMAIL, file_name)
print("Midday mail sent successfully via Brevo")
