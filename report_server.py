...
def rows_to_csv_bytes(rows):
    ...
    writer.writerow(["Date", "Client Name", "Client ID", "Client Status",
                     "Report Sent Time", "Generated By", "Report Status"])
        ...
        try:
            ...
            for log in r3.json().get("data", {}).get("result", []):
                ...
                if sub == "sendReport":
                    ...
                    prev = generate_log.get(cid)
                    if prev is None or log["createdAt"] > prev["createdAt"]:
                        generate_log[cid] = log
        except Exception as e:
            print(f"  Warning: log fetch failed for {cid}: {e}")
    print(f"  Send logs: {len(send_log)}  Generate logs: {len(generate_log)}")
    return client_map, status_map, send_log, generate_log
...
def fetch_opensearch_all(input_date):
    ...
    for log in generate_logs:
        ...
        if not cid or cid in EXCLUDED_IDS:
            continue
        uname = log.get("userName", "Automatic") or "Automatic"
        if cid not in client_info:
            client_info[cid] = {"name": ld.get("reportName", f"Client {cid}"),
                                 "time_val": "-", "generated_by": uname, "has_send": False}
        else:
            existing = client_info[cid].get("generated_by", "Automatic")
            if existing == "Automatic" and uname != "Automatic":
                client_info[cid]["generated_by"] = uname
    ...
    for log in send_logs:
        ...
        if cid not in client_info:
            client_info[cid] = {"name": ld.get("reportName", f"Client {cid}"),
                                 "time_val": time_val, "generated_by": "Automatic", "has_send": True}
        else:
            client_info[cid].update({"time_val": time_val, "has_send": True})
    return client_info


# ── BUILD ROWS FROM DASHBOARD DATA ────────────────────────────────────────────
def build_rows_dashboard(input_date, client_map, status_map, send_log, generate_log):
    rows = []
    for cid, name in client_map.items():
        ...
        if report_status == "Sent":
            gen_entry    = generate_log.get(cid, {})
            generated_by = gen_entry.get("userName", "Automatic") or "Automatic"
        else:
            generated_by = "-"
        rows.append([input_date, name, cid, client_status, time_val, generated_by, report_status])
    return rows
...
<tr style="background-color:#f2f2f2;">
  ...
  <th>Report Sent Time</th><th>Generated By</th><th>Report Status</th>
</tr>
"""
...
def build_summary_html(rows, input_date, last_checked, token_expired, note=""):
    ...
    auto_gen   = sum(1 for r in rows if r[6] == "Sent" and r[5] == "Automatic")
    manual_gen = sum(1 for r in rows if r[6] == "Sent" and r[5] not in ("Automatic", "-"))
...
<tr style="background-color:#f2f2f2;">
  ...
  <th>Auto Generated Count</th><th>Manually Generated Count</th>
</tr>
<tr>
  <td>{total}</td>
  <td style="background-color:#d4edda;">{sent}</td>
  <td style="background-color:#f8d7da;">{pending}</td>
  <td style="background-color:#cce5ff;">{auto_gen}</td>
  <td style="background-color:#fff3cd;">{manual_gen}</td>
</tr>
</table><br><br>
"""
    return html
...
def run_early_report(recipients=None):
    ...
    token_expired = False
    try:
        client_map, status_map, send_log, generate_log = fetch_dashboard_all(input_date, input_date_iso)
        rows = build_rows_dashboard(input_date, client_map, status_map, send_log, generate_log)
        ...
        if e.response is not None and e.response.status_code in (401, 403):
            ...
            rows = [[input_date, i["name"], cid, "Active", i["time_val"],
                     i.get("generated_by", "Automatic"), "Sent" if i["has_send"] else "Pending"]
